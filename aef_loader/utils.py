...
def normalize_embeddings(
    ...
    """
    L2-normalize embeddings along the channel axis.

    This is useful for computing cosine similarity between embeddings.

    Args:
        data: Embedding data with shape (..., channels, H, W)
        axis: Axis to normalize along (default: 0 for channel)

    Returns:
        L2-normalized embeddings
    """
    if isinstance(data, xr.DataArray):
        norm = np.sqrt((data**2).sum(dim="channel"))
        return data / norm
    else:
...
def compute_embedding_similarity(
    emb1: np.ndarray | xr.DataArray,
    emb2: np.ndarray | xr.DataArray,
) -> np.ndarray | xr.DataArray:
    """
    Compute cosine similarity between two embedding arrays.

    Both arrays should have shape (channels, H, W) or be broadcastable.
    ...
    Returns:
        Similarity array with shape (H, W), values in [-1, 1]
    """
    # Normalize
    ...
    if isinstance(emb1_norm, xr.DataArray):
        return (emb1_norm * emb2_norm).sum(dim="channel")
    else:
        return np.sum(emb1_norm * emb2_norm, axis=0)
...
def bbox_to_utm_zone(bbox: tuple) -> int:
    """
    Get UTM EPSG code for a bounding box.

    Uses the center of the bbox to determine the zone.
    ...
    return wgs84_to_utm(center_lon, center_lat)
